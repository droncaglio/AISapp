...
public class AppManagerAISapp : MonoBehaviour {

    public int numeroDeTargets;
    ...
    public void ActionSite(string urlSites)
    {
        Application.OpenURL(urlSites);
    }
    ...
    void Start () {
        ...
        jaBaixou = new bool[numeroDeTargets];
    ...
    public void TrackableFound(int index, DefaultTrackableEventHandlerAISapp imageTarget, string urlAsset, string nameAsset, string urlSite)
    {
        ...
        if (jaBaixou[index])
        {
            imageTarget.transform.GetChild(0).gameObject.SetActive(true);
        }
        else
        {
            if (temInternet)
            {
                StartCoroutine(DownloadAsset(index, imageTarget, urlAsset, nameAsset));
            }
            else
            {
                StartCoroutine(checkInternetConnection((isConnected) => {
    ...
    public void TrackableLost(DefaultTrackableEventHandlerAISapp imageTarget)
    {
        imageTarget.transform.GetChild(0).gameObject.SetActive(false);
    ...
    IEnumerator DownloadAsset(int index, DefaultTrackableEventHandlerAISapp imageTarget, string urlAsset, string nameAsset)
    {
        panelLoading.SetActive(true);
        panelLoading.transform.parent = imageTarget.transform;
        panelLoading.transform.localScale = new Vector3(1, 1, 1);

        WWW www = new WWW(urlAsset);
        yield return www;
        AssetBundle assetBundle = www.assetBundle;
        
        AssetBundleRequest request = assetBundle.LoadAssetAsync(nameAsset);
        ...
        g = Instantiate(assetBundle.LoadAsset(nameAsset) as GameObject);
